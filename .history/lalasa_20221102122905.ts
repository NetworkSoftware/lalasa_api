...
app.post('/prisma/lalasa/login', async (req, res) => {
  ...
  if (phone && password) {
    ...
    if (result) {
      ...
      if (result) {
        res.json({ "data": result, "message": "Welcome to Pet Care.", "success": true });
...
app.put('/prisma/lalasa/assign_service', async (req, res) => {
  ...
  if (sbId && orderId && status) {
    ...
    if (result) {
      ...
      pushNotification(resultupdate.orderType + ' Service', "New Service available", "key=" + sbKey, '/topics/allDevices_' + sbId)
